...
local run_page_code = function(notes_only)
  ...
  if initializing == false then
    ...
    if pages.index == 1 or pages.index == 2 then check_plant_position() end
      ...
      elseif menu_status == false then 
        ...
        if label_obj[2] then
          ...
          elseif label_obj[2] == 3 then
            ...
            screen.move(cursor_start_x,9)
            screen.line_rel(cursor_width+2,0)
          -- elseif label_obj[2] == 4 then
          --   cursor_start = string_cut(label, 1, star_loc+3)
          --   cursor_start_x = screen.text_extents(cursor_start)
          --   cursor_end_x = label
          --   cursor_width = screen.text_extents(cursor_end_x)  - cursor_start_x
          --   screen.move(cursor_start_x,9)
          --   screen.line_rel(cursor_width+5,0)
          end
          screen.stroke()
        end
