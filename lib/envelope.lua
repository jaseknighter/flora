...
local get_envelope_arrays = function (envelope)
  local env_data = {}
...
function Envelope:new(id, num_plants, env_nodes)
  ...
  e.x_min = 0
  e.x_max = MAX_AMPLITUDE
  e.y_min = 0
  e.y_max = MAX_ENV_LENGTH
  ...
  e.env_time_min = 0.01
  e.env_time_max = 2 -- DO NOT CHANGE
  e.curve_min = -50.0
  e.curve_max = 50.0
  ...
  default_graph_nodes[1].time = 0
  default_graph_nodes[1].level = 0
  default_graph_nodes[1].curve = 0
  default_graph_nodes[2] = {}
  default_graph_nodes[2].time = 0
  default_graph_nodes[2].level = 1
  default_graph_nodes[2].curve = -10
  default_graph_nodes[3] = {}
  default_graph_nodes[3].time = 0.5
  default_graph_nodes[3].level = 0.2
  default_graph_nodes[3].curve = -10
  default_graph_nodes[4] = {}
  default_graph_nodes[4].time = 1
  default_graph_nodes[4].level = 0.75
  default_graph_nodes[4].curve = -10
  default_graph_nodes[5] = {}
  default_graph_nodes[5].time = 1.5
  default_graph_nodes[5].level = 0
  ...
  e.set_env_time = function(env_time_max)
    ...
    local old_time_max = e.env_time_max and e.env_time_max or 0.5
    local min_time = alt_key_active and e.env_time_min or 0.5
  ...
  e.update_envelope = function()
    ...
    local env_arrays = get_envelope_arrays(e.graph_nodes)
    
    -- note: to prevent warning messages when changing the number of envelope segments 
    --        (warnings like: "warning: wrong count of arguments for command 'set_levels'")
    --        the envelope arrays are filled in  with zeros.
    --        these zero values will be ignored by the engine
    add_nil_values_to_array(env_arrays.levels,20)
    add_nil_values_to_array(env_arrays.times,20)
    add_nil_values_to_array(env_arrays.curves,20)
  
    engine.set_levels(table.unpack(env_arrays.levels))
    engine.set_times(table.unpack(env_arrays.times))
    engine.set_curves(table.unpack(env_arrays.curves))
  end
  ...
  e.enc = function(n, delta, alt_key_active)
    ...
    elseif n == 3 then 
      ...
      elseif (e.active_node == 0) then
        ...
        -- local old_time_max = e.env_time_max
        -- local min_time = alt_key_active and e.env_time_min or 0.5
        -- e.env_time_max = util.clamp(e.env_time_max + delta_clamp, min_time, e.x_max)
        -- for i=1, #e.graph_nodes, 1
        --   do
        --   local node_time = e.graph_nodes[i].time
        --   e.graph_nodes[i].time = node_time * (e.env_time_max / old_time_max)
      ...
      e.graph:edit_graph(e.graph_nodes)
      clock.run(update_engine, e.graph_nodes)
    end
  end
  ...
  e.key = function(n, delta, alt_key_active)
    ...
    elseif n == 3 and e.active_node >= 1 then
      ...
      elseif (e.active_node < #e.graph_nodes and #e.graph_nodes < 20) then
